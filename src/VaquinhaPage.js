...
export function VaquinhaPage() {
  ...
  useEffect(() => {
    ...
    vaquinhaContract.on("MetaAtingida", (totalArrecadado, metaAlvo, event) => {
  ...
  const doar = async () => {
    ...
    try {
      ...
      const novoTotalArrecadado = await vaquinhaContract.totalArrecadado();
  ...
  const totalArrEmEther = parseFloat(ethers.utils.formatEther(totalArrecadado));
  ...
  const isOrganizer =
      currentUser && organizador && currentUser.toLowerCase() === organizador.toLowerCase();
